...
public OnPluginStart()
{
    ...
    HookEvents();
    RegConsoleCmd("sm_mvps", Command_Kills, "MVP Statistic");
    RegConsoleCmd("sm_mvpss", Command_KillsMe, "MyKills Statistic");
}
...
public KillsStatistic()
{
    ...
    for (new i = 1; i <= MaxClients; i++)
    {
		...
		{
			Format(line, sizeof(line), "{green}★ {default}被拉: {olive}%d {default}被扑: {olive}%d {default}被吐: {olive}%d {default}被撞: {olive}%d {default}被骑: {olive}%d {default}；{olive}%N ", iSmoker[i], iHunter[i], iBoomer[i], iCharger[i], iJockey[i], i);
...
public MyKillsStatistic(Client)
{
	...
	{
		decl String:line[256];
		Format(line, sizeof(line), "{green}★ {default}被拉: {olive}%d {default}被扑: {olive}%d {default}被吐: {olive}%d {default}被撞: {olive}%d {default}被骑: {olive}%d {default}；{olive}%N", iSmoker[Client], iHunter[Client], iBoomer[Client], iCharger[Client], iJockey[Client], Client);
		CPrintToChat(Client, line);
                Format(line, sizeof(line), "{green}★ {default}拳头: {olive}%d {default}石头: {olive}%d {default}；{olive}%N", iTankClaw[Client], iTankRock[Client], Client);
        	CPrintToChat(Client, line);
	}
	else
	{
