...
public OnPluginStart()
{
	...
	selfstandup_health_incap1	= CreateConVar("selfstandup_health_incap1",	"80",		"倒地消耗 止疼药 自救后有多少临时血量");
	selfstandup_health_incap2	= CreateConVar("selfstandup_health_incap2",	"80",		"倒地消耗 肾上腺素 自救后有多少临时血量");
	selfstandup_health_incap3	= CreateConVar("selfstandup_health_incap3",	"100",		"倒地消耗 医疗包 自救后有多少临时血量");
	selfstandup_health_incap4	= CreateConVar("selfstandup_health_incap4",	"100",		"倒地消耗 电击器 自救后有多少临时血量");
	selfstandup_health_incap5	= CreateConVar("selfstandup_health_incap5",	"50",		"倒地消耗 高爆弹药盒 自救后有多少临时血量");
	selfstandup_health_incap6	= CreateConVar("selfstandup_health_incap6",	"50",		"倒地消耗 燃烧弹药盒 自救后有多少临时血量");
	
	selfstandup_duration			= CreateConVar("selfstandup_duration",			"5",		"自救所需时间(限制：0 ~ 15秒之间 )");
	selfstandup_accelerate		= CreateConVar("selfstandup_accelerate",		"1",			"是否启用自救加速功能？0=关闭 1=开启");
	selfstandup_acceleEffect	= CreateConVar("selfstandup_acceleEffect",	"0.5",		"自救加速功能使用一次缩减多少秒？ 0.0=禁用");
	
	selfstandup_crawl					= CreateConVar("selfstandup_crawl",					"0",			"玩家是否可在倒地时爬行？0=关闭 1=开启");
	selfstandup_crawl_speed		= CreateConVar("selfstandup_crawl_speed",		"50",			"设置玩家倒地时爬行速度");
	...
	//selfstandup_clearance			= CreateConVar("selfstandup_clearance",			"0",	"多大范围内有感染者不能自救 最大半径扫描范围 0=关闭");
	selfstandup_teamdistance	= CreateConVar("selfstandup_teamdistance",	"100",	"倒地玩家允许拉起倒地队友的最大距离 0=关闭");
...
public EVENT_HealSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
	...
	if ( IsValidSurvivor( victim ))
	{
		...
		{
			//PrintToChat( victim, "\x04★ \x05当前倒地次数：%d , 第 %d 次倒地后黑白.", RevCount[victim], g_blackwhite );
		}
	}
}
	...
	if ( IsValidSurvivor( victim ))
	{
		...
		else
		{
			...
			if ( GetEntProp( victim, Prop_Send, "m_bIsOnThirdStrike" ) > NONE )
			{
				...
				if ( g_message > NONE )
				{
					//PrintToChatAll( "\x04★ \x05%N \x01处于黑白状态.", victim );
				}
			}
			else
			{
				if ( g_message > NONE )
				{
					PrintToChat( victim, "\x04★ \x05%N \x01帮助了你.", helper );
				}
			}
		}
	}
	...
	if(GetConVarInt(selfstandup_accelerate) != 0)
	{
		...
		if(IsValidSurvivor(client))
		{
			if(press && client > NONE && ( buttons & IN_DUCK ))
			{
				...
				//PrintToChat( client, "\x04★ \x05当前自救数值：%.1f , 加速数值: %.1f.", StartTime[client], AcceleEffect[client]);
			}
		}
	}
	if ( !Button[client] )
	{
		if ( client > NONE && ( buttons & IN_DUCK ))
		{
			...
			if ( target != -1 && IsValidSurvivor( target ) && g_teamdistance > NONE_F && ProgressionTeam( client, target ) == true )
			{
				...
				PrintHintText( client, "★ 你选择的救助目标是 %N ★", target );
				return Plugin_Continue;
			}
		...
		else
		{
			...
			if ( Incap == true )
			{
				...
				else
				{
					...
					{
						if ( g_costly == NONE )
							PrintToChat( victim, "", RevCount[victim], g_blackwhite );
						else
							UsePack( victim, true );
					}
					else
	...
	if ( IsValidSurvivor( victim ))
	{
		...
		if ( Incap == true )
		{
			...
			else
			{
				...
				{
					PrintToChat( victim, "\x04★ \x05%N \x01帮助了你.", helper );
					PrintToChat( helper, "\x04★ \x01成功拉起了 \x05%N\x01.", victim );
				}
			}
		}
		else
		{
			...
			if ( g_message > NONE )
			{
				PrintToChat( victim, "\x04★ \x05%N \x03帮助了你.", helper );
				PrintToChat( helper, "\x04★ \x01成功拉起了 \x05%N\x01.", victim );
			}
		}
		EmitSoundToClient( victim, SOUND_GETUP );
...
ShowBar( victim, Float:pos, Float:max )	 
{
	if ( IsValidSurvivor( victim ))
	{
		...
		for ( new m = NONE; m < 99; m++ )
		{
			ChargeBar[m] = Gauge1[NONE];
		}
		new p = RoundFloat( GaugeNum );
		if ( p >= NONE && p < 99 ) ChargeBar[p] = Gauge3[NONE]; 
		PrintCenterText( victim, "                                << ...正在进行[自救/互救]中... >> %3.0f% \n<<<%s>>>", GaugeNum, ChargeBar );
	}
}
			...
			if ( IsValidEntity( i ))
			{
				if ( IsValidSpecInfected( i ) || IsValidTank( i ))
				{
					...
					if ( GetVectorDistance( targetPos, playerPos ) <= g_clearance)
					{
						...
						if ( g_message > NONE )
						{
							//PrintHintText( victim, "★ 你离 %N 太近了 ★", i );
						}
						break;
					}
				}
				else
				{
					...
					if ( StrEqual( InfName, "infected", false ))
					{
						...
						if ( GetVectorDistance( targetPos, playerPos ) <= g_clearance)
						{
							...
							if ( g_message > NONE )
							{
								//PrintHintText( victim, "★ 你离 %s 太近了 ★", InfName );
							}
							break;
						}
					}
...
stock CallOnPummelEnded( victim, String:GAMEDATA[] )
{
	...
	{
		PrintToServer( "无法获取 CTerrorPlayer::OnPounceEnd SDKCall" );
	}
}
...
UsePack( client, bool:Msg )
{
	if ( PlayerWeaponSlot[client] != -1 && IsValidEntity( PlayerWeaponSlot[client] ))
	{
		...
		if ( StrEqual( slotName, "weapon_adrenaline", false ))
			...
			Format( slotName, sizeof( slotName ), "止痛药" );
		...
		if ( Msg )
		{
			PrintToChat( client, "\x04★★★ \x01使用了 \x05%s \x01自救.", slotName );
		}
