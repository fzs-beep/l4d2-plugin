...
{
	g_hMoreMedical = CreateConVar("l4d2_more_medical", "3", "把需要启用的功能数字相加. 0=禁用, 1=声音, 2=倍数, 4=提示.", CVAR_FLAGS);
...
void IsUpdateEntCount(int client, int g_Multiple, bool g_bPrompt, bool g_bContent, bool g_bMoreCheck, int g_iClientNumber, int g_iSurvivorLimit)
{
	...
	if(g_iMoreMedical & SurvivorsPrompt && g_bMoreCheck)
	{
		if(g_bPrompt)
			...
			PrintToChatAll("\x04[提示]\x05当前人数为\x04(\x03%i\x05/\x03%d\x04)\x03,\x05更改为\x03%s\x05倍医疗物品.", g_iClientNumber, g_iSurvivorLimit, g_sMedical);
	}
}
