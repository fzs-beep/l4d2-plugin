...
public void OnPluginStart()
{
	...
	g_hgive1		= CreateConVar("l4d2_multislots_Survivor_spawn1",				"1",		"启用给予主武器? 0=禁用, 1=随机获得主武器(开局时都没有主武器则不给).", CVAR_FLAGS);
	g_hgive2		= CreateConVar("l4d2_multislots_Survivor_spawn2",				"1",		"启用给予副武器? 1=获得副武器.", CVAR_FLAGS);
	...
	g_hCheck		= CreateConVar("l4d2_multislots_enabled_afk_check",				"0",		"设置幸存者发呆多长时间后提示 !afk 闲置指令可用/秒(最小值:10). 0=禁用.", CVAR_FLAGS);
...
public void Event_PlayerTeam(Event event, const char[] name, bool dontBroadcast)
{
	...
	if(client && !IsFakeClient(client))
	{
		if(g_iTeam != 0 && g_iTeam == 1)
		{
			...
			{
				if(team == 1)
					PrintToChatAll("\x04★ \x05%N \x01加入了观察者.", client);
				else if(team == 2)
					PrintToChatAll("\x04★ \x05%N \x01加入了幸存者.", client);
				else if(team == 3)
					PrintToChatAll("\x04★ \x05%N \x01加入了感染者.", client);
			}
		}
		if(oldteam == 2)
		{
...
public Action AddBot(int client, int args)
{
	if(bCheckClientAccess(client) && iGetClientImmunityLevel(client) >= 98)
	{
		...
		{
			vSpawnFakeSurvivorClient();
			PrintToChat(client, "\x04★ \x05添加电脑成功.");
		}
		else
		{
			if (TotalSurvivors() < g_iSLimit)
			{
				...
				PrintToChat(client, "\x04★ \x05当前无需添加电脑.");
		}
	}
	else
			...
			if(GetClientTeam(i) == 1 && !iGetBotOfIdle(i))
			{
				...
				if(l4d2_gamemode()!=2)
					if(!MenuFunc_SpecNext[i])
						PrintToChat(i, "\x04★ \x05%s \x01输入 \x05!jg \x01或 \x05!join \x01或 \x05按鼠标左键 \x01加入幸存者.", PlayerName);
					else
						PrintToChat(i, "\x04★ \x05%s \x01聊天窗输入 \x05!jg \x01或 \x05!join \x01加入幸存者.", PlayerName);
			}
...
void l4d2_GiveWeapon_pistol_2(int client)
{
	switch(GetRandomInt(0,0))
	{
		...
		case 2:
			BypassAndExecuteCommand(client, "give", "knife");//小刀
		case 3:
			BypassAndExecuteCommand(client, "give", "machete");//砍刀
		case 4:
			BypassAndExecuteCommand(client, "give", "katana");//武士刀
		case 5:
			BypassAndExecuteCommand(client, "give", "fireaxe");//斧头
	}
}
...
void l4d2_GiveWeapon_pistol_1(int client)
{
	switch(GetRandomInt(0,7))
	{
		...
		case 1:
			BypassAndExecuteCommand(client, "give", "autoshotgun");//1代的连发散弹枪
		case 2:
			BypassAndExecuteCommand(client, "give", "smg_silenced");//消声器冲锋枪
		case 3:
			BypassAndExecuteCommand(client, "give", "pumpshotgun");//1代的单发散弹枪
		case 4:
			BypassAndExecuteCommand(client, "give", "rifle");//M16步枪
		case 5:
			BypassAndExecuteCommand(client, "give", "shotgun_chrome");//2代的单发散弹枪
		case 6:
			BypassAndExecuteCommand(client, "give", "rifle_ak47");//AK47步枪
		case 7:
			BypassAndExecuteCommand(client, "give", "shotgun_spas");//2代的连发散弹枪
	}	
}
...
{ 
	...
	{
		case 0:
			PrintToChat(client, "\x04★ \x05加入旁观者指令已禁用,请在CFG中设为1启用.");
		case 1,2:
		{
			if(GetClientTeam(client) == 1)
				PrintToChat(client, "\x04★ \x01你已经是观察者.");
			else if(GetClientTeam(client) == TEAM_SURVIVOR)
			{
				...
				else if(g_iAway == 2)
				{
					if(bCheckClientAccess(client) && iGetClientImmunityLevel(client) >= 98)
						...
						ReplyToCommand(client, "\x04★ \x05加入旁观者指令只限管理员使用.");
				}
			}
		}
	}
...
public Action Command_kickbot(int client, int args) 
{
	if(bCheckClientAccess(client) && iGetClientImmunityLevel(client) >= 98)
	{
		...
		{
			case 0:
				PrintToChat(client, "\x04★ \x05踢出全部电脑幸存者指令已禁用,请在CFG中设为1启用.");
			case 1:
			{
				for (int i = 1; i <= MaxClients; i++)
				{
				...
				PrintToChat(client, "\x04★ \x05已踢出所有电脑.");//此提示使用指令的玩家可见.
			}
		}
	}
	else
...
public Action Command_sset(int client, int args)
{
	if(bCheckClientAccess(client) && iGetClientImmunityLevel(client) >= 98)
	{
		...
		{
			case 0:
				PrintToChat(client, "\x04★ \x05设置服务器人数指令已禁用,请在CFG中设为1启用.");
			case 1:
...
public int SLMenuHandler(Menu menu, MenuAction action, int client, int itemNum)
{
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			PrintToChatAll("\x04★ \x05更改服务器的最大人数为 \x04%i \x05人.", iUserids);
...
public int BotSetMenuHandler(Menu menu, MenuAction action, int client, int itemNum)
{
	switch(action)
	{
		...
		case MenuAction_Select:
		{
			...
			delete g_hBotsUpdateTimer;
			g_hBotsUpdateTimer = CreateTimer(1.0, Timer_BotsUpdate);
			ReplyToCommand(client, "\x04★ \x05已更改幸存者数量为 \x04%d \x05人.", iSurvivor);
		...
		if (IsPlayerAlive(client) && GetClientTeam(client) == 2)
		{
			...
			else
			{
				if (IsPlayerFallen(client))
					PrintToChat(client,"\x04★ \x05倒地时无法使用休息指令.");
				else if (IsPlayerFalling(client))
					PrintToChat(client,"\x04★ \x05挂边时无法使用休息指令.");
				else
				{
					if(gGoAwayFromKeyboard() > 1)
						FakeClientCommand(client, "go_away_from_keyboard");
					else
						PrintToChat(client,"\x04★ \x05只有一名真实幸存者时无法使用休息指令.");
				}
			}
		}
		else if(GetClientTeam(client) == 1)
			PrintToChat(client,"\x04★ \x05你当前已加入了旁观者.");
		else if(!IsPlayerAlive(client))
			PrintToChat(client,"\x04★ \x05死亡状态无权使用休息指令.");
		else
			PrintToChat(client,"\x04★ \x05休息指令只限幸存者使用.");
	}
	return Plugin_Handled;
}
