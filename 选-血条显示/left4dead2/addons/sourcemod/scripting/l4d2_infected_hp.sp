...
public void OnPluginStart()
{
	...
	hWitch 			= CreateConVar("l4d2_infectedhp_witch", 		"0", 		"启用女巫血条显示? 0=禁用, 1=启用.", CVAR_FLAGS);
				...
				if(GetConVarInt(hPluginTank) != 0)
				{
					for(int i=1; i<=max; i++)
					{
						if(IsClientConnected(i) && IsClientInGame(i) && !IsFakeClient(i) && GetClientTeam(i) != 3)
						{
							...
							{
								PrintToChat(i, "\x04★★★ \x05坦克%s \x01突然失去了梦想,自杀身亡.", clName1);//聊天窗提示.
							}
							if(GetConVarInt(hPluginTank) == 2 || GetConVarInt(hPluginTank) == 3)
							{
								PrintHintText(i, "[提示] 坦克%s 突然失去了梦想,自杀身亡.", clName1);//屏幕中下提示.
							}
						}
					}
				}
