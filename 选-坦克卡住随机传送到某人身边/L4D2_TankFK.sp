	...
	if (IsValidEdict(client) && IsValidPlayer(client))
	{
		if(GetEntProp(client, Prop_Send, "m_zombieClass") == 8)
		{
			...
			if (distance <= fkdistance)
			{
				...
				PrintHintTextToAll("检测到坦克被卡,将瞬移到某个倒霉蛋身边!");
			}
			aTimer(client);
		}
	}
...
{
	new Handle:array = CreateArray(1, 0);
	for(new i = 1; i <= MaxClients; i++)
	{
		if(!IsValidPlayer(i) || !IsValidEntity(i) || !IsPlayerAlive(i) || GetClientTeam(i) != 2) continue;
	...
	if (GetArraySize(array) <= 0) return 0;
