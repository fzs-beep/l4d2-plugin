...
public Action afkJoinHint (Handle Timer, int client)
{
	...
	if (client && IsClientInGame(client) && afkPlayerTimeLeftWarn[client] > 0)
	{
		...
		if (GetClientTeam(client) == 1)
		{
			...
			PrintHintText(client, "›î»Î !join º”»Îﬂ[ëÚ...");
						...
						if (L4D2_GetInfectedAttacker(i) == -1)
						{
							...
							if (afkPlayerTimeLeftWarn[i] > 0) // warn time ...
							{
								...
								if (afkPlayerTimeLeftWarn[i] <= 0)
								{
									...
									// We warn the player ....
									PrintHintText(i, "[AFK] Ç…úyÈf÷√! ƒ„å¢Ï∂ %i √Î··èä÷∆≈‘”^.", afkPlayerTimeLeftAction[i]);
								}
							}
							else // player warn timeout reached ...
							{
								...
								if (afkPlayerTimeLeftAction[i] <= 0)
								{
									...
									{
										PrintHintText(i, "[AFK] Ç…úyÈf÷√! “ªµ©”–»ÀÎxÈ_∞≤»´Ö^”Úƒ„å¢èä÷∆≈‘”^.");
									}
								}
			...
			else if (afkKickEnabled)  // if player is on spectators and kick on spectators is enabled ...
			{
				...
				if (!IsClientAdmin(i))
				{
					...
					if (afkPlayerTimeLeftWarn[i] > 0) // warn time ...
					{
						...
						{
							// We warn the player ....
							PrintHintText(i, "[AFK] Ç…úy≈‘”^Èf÷√! ›î»Î!joinº”»Îﬂ[ëÚ\n ∑ÒÑtƒ„å¢Ï∂ %i √Î··Ãﬂ≥ˆÀ≈∑˛∆˜.", afkPlayerTimeLeftAction[i]);
						}
					}
					else // player warn timeout reached ...
					{
...
void afkKickClient (int client)
{
	...
	// Then we kick him
	KickClient(client, "(≈‘”^Èf÷√Ã´æ√±ªÃﬂ) Kicked because of AFK status");
	
	// Print forced info
	char PlayerName[200];
	GetClientName(client, PlayerName, sizeof(PlayerName));
	
	PrintToChatAll("\x01\x04[TS] \x03%s \x01≈‘”^Èf÷√Ã´æ√±ªÃﬂ≥ˆÀ≈∑˛∆˜.", PlayerName);
}
