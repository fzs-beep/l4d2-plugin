...
public OnPluginStart()
{
        ...
        h_cvarPrintType = CreateConVar("l4d_bandw_type", "0", "0 聊天框通知, 1 中间框通知.", FCVAR_PLUGIN, true, 0.0, true, 1.0);
        h_cvarGlowEnable = CreateConVar("l4d_bandw_glow", "1", "0 没有发光轮廓, 1 有发光轮廓", FCVAR_PLUGIN, true, 0.0, true, 1.0);
        h_cvarNoticeType = CreateConVar("l4d_bandw_notice", "2", "0 关闭通知, 1 仅通知幸存者, 2 通知所有人, 3 仅通知感染者.", FCVAR_PLUGIN, true, 0.0, true, 3.0);
...
public EventReviveSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
    if(GetEventBool(event, "lastlife"))
    {
                ...
                {
                        if(bandw_type == 1) PrintHintTextToAll("\x04[提示] \x03%s \x01进入了黑白状态,现在正在发 \x04[白光] \x01谁来帮忙.", targetName, charName);
                        else PrintToChatAll("\x04[提示] \x03%s \x01进入了黑白状态,现在正在发 \x04[白光] \x01谁来帮忙.", targetName, charName);
                }
                //print to infected
                else if(bandw_notice == 3)
                {
                        for( new x = 1; x <= GetMaxClients(); x++)
                        {
                                ...
                                continue;
                                if(bandw_type == 1) PrintHintText(x, "\x04[提示] \x03%s \x01进入黑白状态,现在正在发 \x04[白光] \x01赶紧干掉.", targetName, charName);
                                else PrintToChat(x, "\x04[提示] \x03%s \x01进入黑白状态,现在正在发 \x04[白光] \x01赶紧干掉.", targetName, charName);
                        }
                }
                //print to survivors
                else
                {
                        for( new x = 1; x <= GetMaxClients(); x++)
                        {
                                ...
                                if(bandw_type == 1) PrintHintText(x, "\x04[提示] \x03%s \x01进入黑白状态,现在正在发 \x04[白光] \x01急需治疗.", targetName, charName);
                                else PrintToChat(x, "\x04[提示] \x03%s \x01进入黑白状态,现在正在发 \x04[白光] \x01急需治疗.", targetName, charName);
                        }
                }        
    }
    return;
...
public EventHealSuccess(Handle:event, const String:name[], bool:dontBroadcast)
{
        ...
        if(bandw_glow && IsClientInGame(healee))
        {        
