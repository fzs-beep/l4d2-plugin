...
public void OnPluginStart()
{
	...
	g_hTankMultiple		= CreateConVar("l4d2_tank_Multiples", 	"1.0;1.5;2.0;2.0",	"设置游戏难度对应的倍数(留空=使用默认值:1.0).", CVAR_FLAGS);
	g_hTankBasics		= CreateConVar("l4d2_tank_minimum", 	"4000",				"设置坦克的基础生命值(四名生还者或以内时).", CVAR_FLAGS);
	g_hTankHealth		= CreateConVar("l4d2_tank_health", 		"1000", 			"设置每多一个生还者坦克所增加的血量.", CVAR_FLAGS);
	
	g_hWitchSwitch		= CreateConVar("l4d2_witch_Switch", 	"1", 				"启用女巫出现时根据生还者人数而增加血量? 0=禁用, 1=启用.", CVAR_FLAGS);
	g_hWitchMultiple	= CreateConVar("l4d2_witch_Multiples", 	"1.0;1.0;1.0;1.0",	"设置游戏难度对应的倍数(留空=使用默认值:1.0).", CVAR_FLAGS);
	g_hWitchBasics		= CreateConVar("l4d2_witch_minimum", 	"1000",				"设置女巫的基础生命值(四名生还者或以内时).", CVAR_FLAGS);
	g_hWitchHealth		= CreateConVar("l4d2_witch_health", 	"100",				"设置每多一个生还者女巫所增加的血量.", CVAR_FLAGS);
...
void IsWitchidPrint(DataPack hPack)
{
	...
	if(IsValidEdict(Witchid))
		PrintToChatAll("\x04★★★ \x05%s\x01出现,难度:\x05%s\x01总共:\x05%d\x01名幸存者,血量:\x04%d", GetWitchName(Witchid), sName, IsCountPlayersTeam(), GetEntProp(Witchid, Prop_Data, "m_iMaxHealth"));//聊天窗提示.
	delete hPack;
}
...
void IsTankPrint(DataPack hPack)
{
	...
	if(IsValidTank(client) && g_iTankPrompt != 0)
	{
		if(g_iTankPrompt & PrintChat)
			PrintToChatAll("\x04★★★ \x05坦克%s\x01出现,难度:\x05%s\x01总共:\x05%d\x01名幸存者,血量:\x04%d", GetSurvivorName(client, true), sName, IsCountPlayersTeam(), GetEntProp(client, Prop_Data, "m_iMaxHealth"));//聊天窗提示.
