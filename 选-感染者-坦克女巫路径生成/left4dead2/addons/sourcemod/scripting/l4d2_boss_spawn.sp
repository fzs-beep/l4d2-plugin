...
public void OnPluginStart()
{
	...
	cvarTotalTanksRandom          = CreateConVar("boss_spawn_total_tanks_rng", "0", "[设置地图上能产生坦克的最大随机值|0=关掉随机数值]");
	cvarTotalWitches              = CreateConVar("boss_spawn_total_witches", "1", "[设置地图上能产生女巫的总数]");
	cvarTotalWitchesRandom        = CreateConVar("boss_spawn_total_witches_rng", "0", "[设置地图上能产生女巫的最大随机值|0=关掉随机数值]");
	...
	cvarRangeMinTank              = CreateConVar("boss_spawn_range_min_tank", "25.0", "[设置生成坦克的最小地图路程|0-100%]", FCVAR_NONE, true, 0.0, true, 100.0);
	cvarRangeMaxTank              = CreateConVar("boss_spawn_range_max_tank", "100.0", "设置生成坦克的最大地图路程|0-100%]", FCVAR_NONE, true, 0.0, true, 100.0);
	cvarRangeMinWitch             = CreateConVar("boss_spawn_range_min_witch", "25.0", "设置生成女巫的最小地图路程|0-100%]", FCVAR_NONE, true, 0.0, true, 100.0);
...
public void Event_PlayerLeftCheckpoint(Event event, const char[] name, bool dontBroadcast)
{
	...
	if((L4D_IsFirstMapInScenario() && !g_bStartTanks && !g_bStartWitches) || (L4D_IsMissionFinalMap() && !g_iFinaleTanks && !g_iFinaleWitches))
	{
		delete g_hTimerCheckFlow;
		return;
	}
	...
	g_bAllowSpawnTanks = (g_bStartTanks && L4D_IsFirstMapInScenario() || !L4D_IsFirstMapInScenario()) && (g_iFinaleTanks == 3 || !L4D_IsMissionFinalMap() || !g_bFinaleStarts && g_iFinaleTanks == 1 );

	g_bAllowSpawnWitches = (g_bStartWitches && L4D_IsFirstMapInScenario() || !L4D_IsFirstMapInScenario()) && (g_iFinaleWitches == 3 || !L4D_IsMissionFinalMap() || !g_bFinaleStarts && g_iFinaleWitches == 1 );
...
public Action TimerCheckFlow(Handle timer)
{
	...
	if(g_bAllowSpawnTanks && (g_iMaxTanks && g_iTankCounter < g_iMaxTanks) && (g_fFlowPlayers >= g_fFlowRangeMinTank && g_fFlowPlayers <= g_fFlowRangeMaxTank))
	{
	...
	if(g_bAllowSpawnWitches && (g_iMaxWitches && g_iWitchCounter < g_iMaxWitches) && (g_fFlowPlayers >= g_fFlowRangeMinWitch && g_fFlowPlayers <= g_fFlowRangeMaxWitch))
	{
