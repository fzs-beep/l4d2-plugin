...
bool g_bBlockSound, g_bCvarAllow, g_bMapStarted, g_bWatchHook;
...
public void OnPluginStart()
{
	...
	Handle hGameData = LoadGameConfigFile("l4d2_pourgas");
	...
	*/
	...
	g_hCvarHint =		CreateConVar(	"l4d2_pourgas_hint",		"0",			"Display hint when picking up gascans? 0=Off, 1=Chat text, 2=Hint box.", CVAR_FLAGS);
	g_hCvarHints =		CreateConVar(	"l4d2_pourgas_hints",		"1",			"How many times to display hints, count is reset each map/chapter.", CVAR_FLAGS);
	g_hCvarHold =		CreateConVar(	"l4d2_pourgas_hold",		"1",			"Should players hold down the RELOAD|ZOOM key to pour? 0=No, 1=Yes.", CVAR_FLAGS );
	...
	CreateConVar(						"l4d2_pourgas_version",		PLUGIN_VERSION, "Pour Gas plugin version.", FCVAR_NOTIFY|FCVAR_DONTRECORD);
...
void HintMessages(int client)
{
	...
	switch ( hint )
	{
		case 1:		PrintToChat(client, "%s%T", CHAT_TAG, "PourGas_Pickup", client);
		case 2:		PrintHintText(client, "%T", "PourGas_Pickup", client);
	}
}
	...
	for( int i = 0; i < MAX_POURS; i++ )
{
	...
	{
		CreateTimer(0.5, TimerBlood, caller, TIMER_FLAG_NO_MAPCHANGE);
	}
}
...
public void OnEntityCreated(int entity, const char[] classname)
{
	if( g_bWatchHook && strcmp(classname, "inferno") == 0 )
	{
		...
		{
			CreateTimer(10.0, TimerFire, EntIndexToEntRef(entity));
		}
	}
}
...
public Action TimerFire(Handle timer, any target)
{
	if( EntRefToEntIndex(target) != INVALID_ENT_REFERENCE )
	{
		...
		if( entity != -1 )
		{
			...
			AcceptEntityInput(entity, "start");
			char sTemp[64];
			Format(sTemp, sizeof(sTemp), "OnUser1 !self:Kill::%f:-1", g_iCvarBurn - 11.0);
			SetVariantString(sTemp);
			AcceptEntityInput(entity, "AddOutput");
			AcceptEntityInput(entity, "FireUser1");
			SetVariantString("!activator");
...
public Action SoundHook(int clients[64], int &numClients, char sample[PLATFORM_MAX_PATH], int &entity, int &channel, float &volume, int &level, int &pitch, int &flags)
{
	...
	{
		if( strncmp(sample, "weapons/molotov/", 16, false) == 0 && (strncmp(sample[16], "fire_ignite", 11, false) == 0 || strncmp(sample[16], "molotov", 7, false) == 0) )
		{
			volume = 0.0;
			return Plugin_Changed;
		}
