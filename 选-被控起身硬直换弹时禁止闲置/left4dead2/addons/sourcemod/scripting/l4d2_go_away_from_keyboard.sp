...
#include <sdktools>
#include <left4dhooks>
#include <l4d2_go_away_from_keyboard>
...
public void OnPluginStart()
{
    ...
    l4d2_awayfromserver    = CreateConVar("l4d2_enabled_away_from_server", "1", "启用被特感控制,发射榴弹,投掷燃烧瓶时禁止使用闲置? 0=禁用, 1=启用.", FCVAR_NOTIFY);
    l4d2_timerkeyboard    = CreateConVar("l4d2_enabled_away_timer_keyboard", "10.0", "设置多少秒后才能再次使用闲置.", FCVAR_NOTIFY);
    ...
    afkConditions = Afk_Blind | Afk_IsTank;
...
{
    RegPluginLibrary("l4d2_go_away_from_keyboard");
    CreateNative("L4D2_Afk_PreventClient", Native_Afk_PreventClient);
    CreateNative("L4D2_Afk_AllowClient", Native_Afk_AllowClient);

    return APLRes_Success;
}


any Native_Afk_PreventClient(Handle plugin, int numParams)
{
    int client = GetNativeCell(1);
    int condition = GetNativeCell(2);
    playerStatus[client] |= condition;
    return 0;
}

any Native_Afk_AllowClient(Handle plugin, int numParams)
{
    int client = GetNativeCell(1);
    int condition = GetNativeCell(2);
    playerStatus[client] &= ~condition;
...
void vFromLoadGameData()
{
    ...
    if(FileExists(QsPath)) 
    {
        ...
        if (!FileExists(QsPath))
        {
            SetFailState("[Error] The required gamedata file 'l4d2_go_away_from_keyboard.txt' is missing in the 'gamedata' directory.");
        }
...
public Action away_from_keyboard(int client, const char[] command, int args)
{
    ...
    if (g_awayfromserver == 1)
    {
        if(IsValidClient(client))
        {
            ...
            {
                if(playerStatus[client] & afkConditions)
                {
                    PrintCenterText(client, "★ 此状态下不能闲置 ★");
                    return Plugin_Handled;
                }
                if (g_iAllBotGame == 0 && GetSurvivorsNumber() <= 1)
                {
                    PrintToChat(client, "★ 必须要两名玩家才能使用闲置指令 ★");
                    return Plugin_Handled;
                }
                if (go_away_from(client))
                {
                    PrintCenterText(client, "★ 被特感控制时禁止闲置 ★");
                    return Plugin_Handled;
                }
                ...
                {
                    PrintCenterText(client, "★ 发射榴弹或投掷燃烧瓶时禁止闲置 ★");
                    return Plugin_Handled;
                }
                else if (fromTime < g_timerkeyboard)
                {
                    PrintCenterText(client, "★ 请等待 %.1f 秒后再使用闲置 ★", g_timerkeyboard - fromTime);
                    return Plugin_Handled;
                }
                else if (IsInReload(client))
                {
                    PrintCenterText(client, "★ 更换弹药时禁止闲置 ★");
                    return Plugin_Handled;
                }
                else if (bIsGettingUp(client))
                {
                    PrintCenterText(client, "★ 起身过程中禁止闲置 ★");
                    return Plugin_Handled;
                }
                else if (L4D_IsPlayerStaggering(client))
                {
                    PrintCenterText(client, "★ 硬直状态时禁止闲置 ★");
                    return Plugin_Handled;
                }
                else 
                {
            ...
            {
                //PrintToChat(client,"\x04★ \x05你当前已经加入了观察者.");
                return Plugin_Handled;
            }
            else if(!IsPlayerAlive(client))
            {
                //PrintToChat(client,"\x04★ \x05死亡状态禁止使用闲置.");
                return Plugin_Handled;
            }
            else if(GetClientTeam(client) == TEAM_INFECTED)
            {
                //PrintToChat(client,"\x04★ \x05闲置指令只限幸存者使用.");
                return Plugin_Handled;
            }
        }
    }
