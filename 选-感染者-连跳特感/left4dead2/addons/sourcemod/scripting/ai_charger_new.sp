...
public void OnPluginStart()
{
	...
	g_hBhopSpeed = CreateConVar("ai_ChagrerBhopSpeed", "90", "Charger 连跳速度", CVAR_FLAG, true, 0.0);
	g_hChargeDist = CreateConVar("ai_ChargerChargeDistance", "250", "Charger 只能在与目标小于这一距离时冲锋", CVAR_FLAG, true, 0.0);
	g_hExtraTargetDist = CreateConVar("ai_ChargerExtraTargetDistance", "0,350", "Charger 会在这一范围内寻找其他有效的目标（中间用逗号隔开，不要有空格）", CVAR_FLAG);
	g_hAimOffset = CreateConVar("ai_ChargerAimOffset", "30", "目标的瞄准水平与 Charger 处在这一范围内，Charger 不会冲锋", CVAR_FLAG, true, 0.0);
	...
	AutoExecConfig(true, "ai_charger_new");
