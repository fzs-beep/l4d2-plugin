...
public void OnPluginStart()
{
	...
	l4d2_timerkeyboard	= CreateConVar("l4d2_enabled_away_timer_keyboard", "2.0", "设置多少秒后才能再次使用休息.", FCVAR_NOTIFY);
	l4d2_timerSpecNext	= CreateConVar("l4d2_enabled_away_timer_specnext", "1.0", "设置多少秒后才能再加入幸存者.", FCVAR_NOTIFY);
...
public Action CommandListener_SpecNext(int client, char[] command, int argc)
{
	if(IsValidClient(client) && !IsFakeClient(client))
	{
		if(GetClientTeam(client) == 1 && iGetBotOfIdle(client))
		{
			...
			{
				PrintHintText(client, "请等待 %.1f 秒后再加入幸存者.", g_timerSpecNext - SpecTime);
