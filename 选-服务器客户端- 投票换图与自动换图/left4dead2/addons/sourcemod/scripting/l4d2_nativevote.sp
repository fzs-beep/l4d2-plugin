...
public void OnPluginStart()
{
	...
	g_cvInitiatorAutoVoteYes = CreateConVar("l4d2_nativevote_initiator_auto_voteyes", "0", "If 1, initiator will auto vote yes", FCVAR_NONE, true, 0.0, true, 1.0);
...
Action cmdPass(int client, int args) {
	if (g_VoteData.bVoteInProgress) {
		...
		g_VoteData.iYesCount = g_VoteData.iPlayerCount;
...
Action cmdVeto(int client, int args) {
	if (g_VoteData.bVoteInProgress) {
		...
		g_VoteData.iNoCount = g_VoteData.iPlayerCount;
		UpdateVotes(VoteAction_End, VOTEEND_FULLVOTED);
	}
