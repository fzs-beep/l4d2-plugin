...
public void OnPluginStart()
{
	...
	// Plugin cvars
	g_hCvarMenu =		CreateConVar(	"l4d_votemode_admin_menu",		"1", 			"0=否,1=是,是否管理菜单的服务器命令中显示.", CVAR_FLAGS );
	g_hCvarAdmin =		CreateConVar(	"l4d_votemode_admin_flag",		"", 			"拥有这些标志的玩家可以投票改变游戏模式.", CVAR_FLAGS );
	g_hCvarReset =		CreateConVar(	"l4d_votemode_reset",			"",				"指定当所有玩家都断开连接时要重置的游戏模式.留空 = 不重置.", CVAR_FLAGS );
	g_hCvarRestart =	CreateConVar(	"l4d_votemode_restart",			"1",			"0=不重新启动,1=使用“changelevel”命令,2=使用“mp_restartgame”cvar重新启动地图.", CVAR_FLAGS );
	g_hCvarTimeout =	CreateConVar(	"l4d_votemode_timeout",			"30",			"投票应可见多长时间.", CVAR_FLAGS, true, 5.0, true, 60.0 );
	...
	RegConsoleCmd(	"sm_vtmd",					CommandVote,					"Displays a menu to vote the game into a different mode.");
	RegConsoleCmd(	"sm_vm",					CommandVote,					"Displays a menu to vote the game into a different mode.");
...
void StartVote(int client, int iMode)
{
	...
	g_smConfigData.GetArray(sTemp, data, sizeof(data));
	L4D2NativeVote vote = L4D2NativeVote(VoteMode_Handler);
	vote.Initiator = client;
	vote.SetInfo(data.sDisplay);
	...
	for (int i = 1; i <= MaxClients; i++) {
		...
		vote.SetTitle("%t %s?", "VoteMode_Change", data.sDisplay);

		clients[0] = i;
		vote.DisplayVote(clients, 1, g_iCvarTimeout);
	}
	// Display vote
	/*for( int i = 1; i <= MaxClients; i++ )
	{
...
*/
void VoteMode_Handler(L4D2NativeVote vote, VoteAction action, int param1, int param2) {
	switch (action) {
		case VoteAction_Start: {
			...
			ExplodeString(buffer, "|", info, 1, 32);//拆分字符串.
			for (int i = 1; i <= MaxClients; i++) {
				if (IsClientInGame(i) && !IsFakeClient(i)) {
					PrintToChat(i, "%s\x03%N\x01发起投票\x05更改模式\x01:\x04%s", CHAT_TAG, param1, info[0]);
				}
			}
		}
		
		case VoteAction_PlayerVoted:
			PrintToChatAll("%s\x03%N\x04已投票", CHAT_TAG, param1);
		...
		case VoteAction_End: {
			if (vote.YesCount > vote.PlayerCount / 2) {
				...
				vote.GetInfo(buffer, sizeof buffer);
				ExplodeString(buffer, "|", info, 1, 32);//拆分字符串.
				vote.SetPass("%t %s", "VoteMode_Voted", info[0]);
				g_bAllVoted = true;
				g_bVoteInProgress = false;
				for (int i = 1; i <= MaxClients; i++) {
					if (IsClientInGame(i) && !IsFakeClient(i)) {
