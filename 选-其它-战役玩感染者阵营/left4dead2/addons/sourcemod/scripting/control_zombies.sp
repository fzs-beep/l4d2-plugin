...
public void OnPluginStart() {
	...
	g_cSurvivorLimit =				CreateConVar("cz_allow_survivor_limit",				"4",					"至少有多少名正常生还者(未被控,未倒地,未死亡)时,才允许玩家接管坦克", CVAR_FLAGS, true, 0.0);
	g_cSurvivorChance =				CreateConVar("cz_survivor_allow_chance",			"0",					"准备叛变的玩家数量为0时,自动抽取生还者和感染者玩家的几率(排除闲置旁观玩家)(0.0=不自动抽取)", CVAR_FLAGS);
	...
	g_cPZPunishHealth =				CreateConVar("cz_pz_punish_health",					"0.0",					"特感玩家在ghost状态下切换特感类型是否进行血量惩罚(0.0=不惩罚.计算方式为当前血量乘以该值)", CVAR_FLAGS, true, 0.0);
	g_cAutoDisplayMenu =			CreateConVar("cz_atuo_display_menu",				"0",					"在感染玩家进入灵魂状态后自动向其显示更改类型的菜单?(0=不显示,-1=每次都显示,大于0=每回合总计显示的最大次数)", CVAR_FLAGS, true, -1.0);
	...
	g_cCmdCooldownTime =			CreateConVar("cz_cmd_cooldown_time",				"60",					"sm_team2,sm_team3命令的冷却时间(0.0-无冷却)", CVAR_FLAGS, true, 0.0);
	...
	g_cLotTargetPlayer =			CreateConVar("cz_lot_target_player",				"1",					"抽取哪些玩家来接管坦克?(-1=由游戏自身控制,0=不抽取,1=叛变玩家,2=生还者,4=感染者)", CVAR_FLAGS);
	...
	AutoExecConfig(true, "control_zombies");
