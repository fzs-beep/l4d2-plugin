...
public void OnPluginStart()
{
	...
	g_hRespawnAdd 			= 	CreateConVar("sar_respawn_survivor_time_add",	"0", 	"玩家每次死亡增加多少复活时间(秒).", CVAR_FLAGS);
	g_hRespawnMax 			= 	CreateConVar("sar_respawn_survivor_time_max",	"60", 	"玩家自动复活的最大时间(秒).", CVAR_FLAGS);

	g_hRespawnLimit 		= 	CreateConVar("sar_respawn_survivor_limit",		"5", 	"玩家每回合自动复活次数.", CVAR_FLAGS);
	...
	g_esWeapon[0].cFlags	=	CreateConVar("sar_respawn_slot0", 	"29", 		"主武器给什么 \n0=不给,131071=所有,7=微冲,1560=霰弹,30720=狙击,31=Tier1,32736=Tier2,98304=Tier0.");
	g_esWeapon[1].cFlags	=	CreateConVar("sar_respawn_slot1", 	"1", 		"副武器给什么 \n0=不给,131071=所有.如果选中了近战且该近战在当前地图上未解锁,则会随机给一把.");
	...
	g_hRemoveDeathDrop		= 	CreateConVar("sar_remove_drop", 	"0", 		"玩家死亡移除掉落的武器. 0=禁用, 1=启用.", CVAR_FLAGS);
