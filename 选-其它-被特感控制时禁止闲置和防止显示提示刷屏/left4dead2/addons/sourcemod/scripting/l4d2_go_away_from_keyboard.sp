...
public void OnPluginStart()
{
	...
	l4d2_timerkeyboard	= CreateConVar("l4d2_enabled_away_timer_keyboard", "10.0", "设置多少秒后才能再次使用休息.", FCVAR_NOTIFY);
	...
	if (g_awayfromserver == 1)
	{
		if(IsValidClient(client))
		{
			if(IsPlayerAlive(client) && GetClientTeam(client) == 2)
			{
				...
				{
					PrintHintText(client, "请等待 %.1f 秒后再使用休息.", g_timerkeyboard - fromTime);
					return Plugin_Handled;
				}
