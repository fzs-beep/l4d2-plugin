...
void PrintTankStatistics(int tank) {
	...
	for (i = 0; i < length; i++) {
		...
		Format(buffer, sizeof buffer, "%s", "{green}★ ");
		...
		Format(buffer, sizeof buffer, "%s%s", buffer, " {olive}(");
		numSpace = percLen - IntToString(percent, str, sizeof str);
		AppendSpaceChar(buffer, sizeof buffer, numSpace);
		Format(buffer, sizeof buffer, "%s{green}%s%%", buffer, str);
		AppendSpaceChar(buffer, sizeof buffer, numSpace);

		Format(buffer, sizeof buffer, "%s%s", buffer, "{olive}) {default}吃拳: ");
		numSpace = clawLen - IntToString(g_esData[tank].tankClaw[client], str, sizeof str);
		AppendSpaceChar(buffer, sizeof buffer, numSpace);
		Format(buffer, sizeof buffer, "%s{olive}%s", buffer, str);
		AppendSpaceChar(buffer, sizeof buffer, numSpace);
