...
public void OnPluginStart() {
	...
	RegConsoleCmd("sm_mapsz",		cmdMapNext);
	RegConsoleCmd("sm_votenext",	cmdMapNext);
	RegConsoleCmd("sm_nextvote",	cmdMapNext);

	RegAdminCmd("sm_mapvpk",		cmdReload,		ADMFLAG_ROOT);
	RegAdminCmd("sm_vpkmap",		cmdReload,		ADMFLAG_ROOT);
...
void OnFinaleStart(const char[] output, int caller, int activator, float delay) {
	if (!g_bOnce && L4D_IsMissionFinalMap()) {
		...
		if (g_iNotifyMapNext & NOTIFY_CHAT)
			PrintToChatAll("\x04★★★ \x01聊天栏输入 \x05!mapnext \x01投票下一张地图.");
...
Action cmdReload(int client, int args) {
	...
	ReplyToCommand(client, "\x04★★★ \x05更新VPK文件 \x04★★★");
