...
public OnPluginStart()
{
	...
	PluginCvarMode = CreateConVar("l4d2_extendedsight_mode", "3", "求生者怎么样获取透视能力？（０＝关闭　１＝当坦克被杀死　２＝当女巫被杀死　３＝当坦克或女巫被杀死　４＝只要活着就会有）", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 4.0);
	PluginCvarNotify = CreateConVar("l4d2_extendedsight_notify", "1", "当击杀坦克与女巫获取透视能力如何通知玩家？（０＝关闭　１＝中下方提示框　２＝聊天框）", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 2.0);
	PluginCvarDuration = CreateConVar("l4d2_extendedsight_duration", "60", "求生者获取这能力持续多少秒？", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 10.0);
	PluginCvarGlow = CreateConVar("l4d2_extendedsight_glowcolor", "255 0 0", "特感发光颜色，用RGB，用空格分隔的数值", FCVAR_PLUGIN|FCVAR_NOTIFY);
	PluginCvarGlowMode = CreateConVar("l4d2_extendedsight_glowmode", "1", "特感发光模式（０＝持续的发光　１＝渐变的发光）", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 0.0, true, 1.0);
	PluginCvarGlowFadeInterval = CreateConVar("l4d2_extendedsight_glowfadeinterval", "1.5", "每个特感发光淡出之间的间隔（秒）", FCVAR_PLUGIN|FCVAR_NOTIFY, true, 1.5, true, 10.0);
	PluginCvarModesOn =	CreateConVar("l4d2_extendedsight_modes_on", "", "那些模式可以开？（空白＝全部）", FCVAR_PLUGIN|FCVAR_NOTIFY);
	PluginCvarModesOff = CreateConVar("l4d2_extendedsight_modes_off", "", "那些模式可以关闭？（空白＝没有）", FCVAR_PLUGIN|FCVAR_NOTIFY);
	
	RegAdminCmd("sm_gh", Command_ExtendedSight, ADMFLAG_BAN, "Extended Survivor Sight On/Off");
