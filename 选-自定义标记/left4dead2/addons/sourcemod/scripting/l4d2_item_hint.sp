...
public void OnPluginStart()
{
	...
	g_hItemCvarCMD					= CreateConVar("l4d2_item_hint_cmd", 							"1", 				"如果为1,玩家可以输入!bj指令为标记", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hItemCvarShiftE				= CreateConVar("l4d2_item_hint_shiftE", 						"1", 				"如果为1,玩家可以按Shift+E进行标记", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hItemCvarVocalize 			= CreateConVar("l4d2_item_hint_vocalize", 						"1", 				"如果为1,玩家可以使用“看”进行标记", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hCappedMark					= CreateConVar("l4d2_item_hint_mark_capped", 					"0", 				"如果为1,玩家可以使用标记,如果被S.I.盯住", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hHaningMark					= CreateConVar("l4d2_item_hint_mark_hanging", 					"0", 				"如果为1,玩家可以使用标记,如果是从窗台上悬挂", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hDeadMark						= CreateConVar("l4d2_item_hint_mark_dead", 						"0", 				"如果为1,玩家可以在已经死亡的情况下使用标记", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hHintTransType				= CreateConVar("l4d2_item_hint_instructorhint_translate", 		"0", 				"所有玩家使用哪种语言显示指令提示文本?0=服务器语言(英语),1=呼叫者语言", FCVAR_NOTIFY, true, 0.0, true, 1.0);

	g_hItemCvarColor				= CreateConVar("l4d2_item_marker_glow_color", 					"0 255 255", 			"物品发光颜色,自行去https://tool.oschina.net/commons?type=3比对颜色(无内容=禁用物品发光)", FCVAR_NOTIFY);
	g_hItemHintCoolDown				= CreateConVar("l4d2_item_marker_cooldown_time", 				"1.0", 					"玩家使用语音菜单的‘看’创建查看物品的冷却时间", FCVAR_NOTIFY, true, 0.0);
	g_hItemUseHintRange				= CreateConVar("l4d2_item_marker_use_range", 					"150", 					"玩家使用语音菜单查看物品最远距离", FCVAR_NOTIFY, true, 1.0);
	g_hItemUseSound					= CreateConVar("l4d2_item_marker_use_sound", 					"buttons/blip1.wav", 	"标记物品提示音(格式一般是:sound/,无内容:禁用)", FCVAR_NOTIFY);
	g_hItemAnnounceType				= CreateConVar("l4d2_item_marker_announce_type", 				"1", 					"物品标记提示的显示方式.(0:禁用,1:在聊天中,2:在提示框中,3:在屏幕中心）", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	g_hItemGlowTimer				= CreateConVar("l4d2_item_marker_glow_timer", 					"15.0", 				"物品发光时间", FCVAR_NOTIFY, true, 0.0);
	g_hItemGlowRange				= CreateConVar("l4d2_item_marker_glow_range", 					"1000", 				"物品发光范围", FCVAR_NOTIFY, true, 0.0);
	g_hItemInstructorHint			= CreateConVar("l4d2_item_marker_instructorhint_enable", 		"1", 					"如果为1,则在物品上方显示物品提示", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hItemInstructorColor			= CreateConVar("l4d2_item_marker_instructorhint_color", 		"0 255 255", 			"标记的项目上提示颜色.(如果为空,则关闭项目名称显示)", FCVAR_NOTIFY);
	g_hItemInstructorIcon			= CreateConVar("l4d2_item_marker_instructorhint_icon", 			"icon_interact", 		"标记上方提示的图标(更多图标查看https://developer.valvesoftware.com/wiki/Env_instructor_hint)", FCVAR_NOTIFY);

	g_hSpotMarkCvarColor			= CreateConVar("l4d2_spot_marker_color", 						"0 255 255", 			"标记发光颜色,0-255之间的三个值,用空格分隔.(空=禁用点标记)", FCVAR_NOTIFY);
	...
	g_hSpotMarkAnnounceType			= CreateConVar("l4d2_spot_marker_announce_type", 				"0", 					"更改标记上方的显示方式.(0:禁用,VC1:聊天中,2:提示框中,3:居中文本)", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	g_hSpotMarkGlowTimer			= CreateConVar("l4d2_spot_marker_duration", 					"15.0", 				"标记持续时间", FCVAR_NOTIFY, true, 0.0);
	g_hSpotMarkSpriteModel      	= CreateConVar("l4d2_spot_marker_sprite_model", 				"materials/vgui/icon_download.vmt", "标记模型(无内容=禁用模型)");
	...
	g_hSpotMarkInstructorIcon		= CreateConVar("l4d2_spot_marker_instructorhint_icon", 			"", 					"标记上方提示图标", FCVAR_NOTIFY);
	...
	g_hInfectedMarkUseSound			= CreateConVar("l4d2_infected_marker_use_sound", 				"items/suitchargeok1.wav", "标记特殊感染者提示音(格式一般是:sound/,无内容:禁用)", FCVAR_NOTIFY);
	g_hInfectedMarkAnnounceType		= CreateConVar("l4d2_infected_marker_announce_type",			"1", 					"更改感染标记提示的显示方式.(0:禁用,1:聊天中,2：提示框中,3:居中文本)", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	g_hInfectedMarkGlowTimer   		= CreateConVar("l4d2_infected_marker_glow_timer", 				"10.0", 				"感染标记发光时间", FCVAR_NOTIFY, true, 0.0);
	g_hInfectedMarkGlowRange   		= CreateConVar("l4d2_infected_marker_glow_range", 				"2500", 				"感染标记发光范围", FCVAR_NOTIFY, true, 0.0);
	g_hInfectedMarkWitch    		= CreateConVar("l4d2_infected_marker_witch_enable", 			"1", 					"如果为1,在开关上启用“看”感染者标记", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hInfectedMarkInstructorHint	= CreateConVar("l4d2_infected_marker_instructorhint_enable", 	"1", 					"如果为1,则在被感染者的头上创建提示(如果标记)", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hInfectedMarkInstructorColor	= CreateConVar("l4d2_infected_marker_instructorhint_color", 	"255 0 0", 				"标记感染者上方提示的颜色.(如果为空,则关闭僵尸类显示)", FCVAR_NOTIFY);
	g_hInfectedMarkInstructorIcon	= CreateConVar("l4d2_infected_marker_instructorhint_icon", 		"", 					"标记感染者上方提示图标", FCVAR_NOTIFY);

	g_hSurvivorMarkCvarColor   		= CreateConVar("l4d2_survivor_marker_glow_color", 				"0 200 0", 					"生还者标记颜色,自行去https://tool.oschina.net/commons?type=3比对颜色(无内容=禁用标记)Empty = Disable Survivor Marker)", FCVAR_NOTIFY);
	g_hSurvivorMarkCoolDown			= CreateConVar("l4d2_survivor_marker_cooldown_time", 			"1.0", 						"玩家使用语音菜单的‘看’标记发光生者的冷却时间", FCVAR_NOTIFY, true, 0.0);
	g_hSurvivorMarkUseRange     	= CreateConVar("l4d2_survivor_marker_use_range", 				"1000", 					"玩家多远可以标记特生还者", FCVAR_NOTIFY, true, 1.0);
	g_hSurvivorMarkUseSound			= CreateConVar("l4d2_survivor_marker_use_sound", 				"player/suit_denydevice.wav",  "标记生还者提示音(格式一般是:sound/,无内容:禁用)", FCVAR_NOTIFY);
	g_hSurvivorMarkAnnounceType		= CreateConVar("l4d2_survivor_marker_announce_type", 			"1", 						"更改幸存者标记提示的显示方式,（0:禁用，1:聊天中,2:提示框中,3:居中文本）", FCVAR_NOTIFY, true, 0.0, true, 3.0);
	g_hSurvivorMarkGlowTimer   		= CreateConVar("l4d2_survivor_marker_glow_timer", 				"10.0", 					"生还者标记发光时间", FCVAR_NOTIFY, true, 0.0);
	g_hSurvivorMarkGlowRange   		= CreateConVar("l4d2_survivor_marker_glow_range", 				"2000", 					"生还者标记发光范围", FCVAR_NOTIFY, true, 0.0);
	g_hSurvivorMarkInstructorHint	= CreateConVar("l4d2_survivor_marker_instructorhint_enable", 	"1", 						"如果为1,在开关上启用“看”生还者标记", FCVAR_NOTIFY, true, 0.0, true, 1.0);
	g_hSurvivorMarkInstructorColor	= CreateConVar("l4d2_survivor_marker_instructorhint_color", 	"0 200 0", 					"标记生还者上方提示的颜色. (如果为空,则关闭名称显示)", FCVAR_NOTIFY);
	g_hSurvivorMarkInstructorIcon	= CreateConVar("l4d2_survivor_marker_instructorhint_icon", 		"", 						"标记生还者上方提示图标", FCVAR_NOTIFY);
	...
	RegConsoleCmd("sm_bj", CMD_MARK, "Mark item/infected/spot");
